...
impl<'a> Parser<'a> {
    ...
    fn primary_expression(&mut self) -> ParseResult<Option<AstNode>> {
        ...
        match self.unchecked_identifier() {
            ...
            Some(b"list") => return Ok(Some(AstNode::ListBuilder(self.list()?))),
            Some(b"map") => return Ok(Some(AstNode::MapBuilder(self.map()?))),
...
pub mod tests {
    use super::*;
    use crate::parser::{map::EXPECTED_OPEN_PARENS, string::MISSING_END_QUOTE, tests::*};
    ...
    fn test_expr_ok() {
        ...
        do_test_expr_err(" map.foo ", 4, EXPECTED_OPEN_PARENS);
    }
